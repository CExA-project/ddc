...
plotter = lambda plt, x_name, y_name, data_dict_sorted, filter: plt.plot(
    [item[x_name] for item in data_dict_sorted if filter(item)],
    [item[y_name] for item in data_dict_sorted if filter(item)],
    marker="o",
    markersize=5,
    label=f"{'non uniform' if any(filter(item) and item['non_uniform'] for item in data_dict_sorted) else 'uniform'} nx={nx}",
)
...
if name == "degree_x":
    ...
    for non_uniform, nx in itertools.product((False, True), nx_values):
        plotter(
            ...
            "bytes_per_second",
            data_dict_sorted,
            lambda item: item["nx"] == nx
            and item["non_uniform"] == non_uniform
            and not item["on_gpu"],
        )
    ...
    plt.title(
        str(backend)
        + ": Throughput on CPU (with ny="
        + str([item["ny"] for item in data_dict_sorted][0])
        + ")"
    )
    plt.legend()
    ...
    for non_uniform, nx in itertools.product((False, True), nx_values):
        plotter(
            ...
            "bytes_per_second",
            data_dict_sorted,
            lambda item: item["nx"] == nx and item["non_uniform"] == non_uniform and item["on_gpu"],
        )
...
if name == "ny":
    ...
    for non_uniform, nx in itertools.product((False, True), nx_values):
        ...
        plt.plot(
            [item["ny"] for item in data_dict_sorted if filter(item)],
            [
                (item["gpu_mem_occupancy"] - nx * item["ny"] * 8) / (nx * item["ny"] * 8) * 100
