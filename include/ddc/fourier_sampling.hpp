...
/** FourierSampling models a fourier discretization of the provided continuous dimension
 */
template <class CDim>
...
public:
    ...
    using discrete_element_type = DiscreteElement<FourierSampling>;

    using discrete_domain_type = DiscreteDomain<FourierSampling>;

    using discrete_vector_type = DiscreteVector<FourierSampling>;
...
public:
    ...
    /** Construct a fourier `DiscreteDomain` from a segment \f$[a, b] \subset [a, +\infty[\f$ and a
     *  number of points `n`.
     *
     * @param a coordinate of the first point of the domain
     * @param b coordinate of the last point of the domain
...
template <class>
struct is_fourier_sampling : public std::false_type
{
};

template <class CDim>
struct is_fourier_sampling<FourierSampling<CDim>> : public std::true_type
{
};

template <class DDim>
constexpr bool is_fourier_sampling_v = is_fourier_sampling<DDim>::value;


template <
        class DDimImpl,
        std::enable_if_t<
                is_fourier_sampling_v<typename DDimImpl::discrete_dimension_type>,
                int> = 0>
std::ostream& operator<<(std::ostream& out, DDimImpl const& mesh)
{
    return out << "FourierSampling( origin=" << mesh.origin() << ", step=" << mesh.step()
               << " )";
}
...
DDC_INLINE_FUNCTION std::
        enable_if_t<is_fourier_sampling_v<DDim>, typename DDim::continuous_element_type>
        origin() noexcept
{
...
template <class DDim>
DDC_INLINE_FUNCTION std::
        enable_if_t<is_fourier_sampling_v<DDim>, typename DDim::discrete_element_type>
        front() noexcept
{
...
/// @brief Spacing step of the mesh
template <class DDim>
DDC_INLINE_FUNCTION std::enable_if_t<is_fourier_sampling_v<DDim>, double> step() noexcept
{
...
DDC_INLINE_FUNCTION constexpr Coordinate<CDim> coordinate(
        DiscreteElement<FourierSampling<CDim>> const& c)
{
    return discrete_space<FourierSampling<CDim>>().coordinate(c);
}

template <class CDim>
DDC_INLINE_FUNCTION Coordinate<CDim> distance_at_left(DiscreteElement<FourierSampling<CDim>>)
{
    return Coordinate<CDim>(step<FourierSampling<CDim>>());
}

template <class CDim>
DDC_INLINE_FUNCTION Coordinate<CDim> distance_at_right(DiscreteElement<FourierSampling<CDim>>)
{
    return Coordinate<CDim>(step<FourierSampling<CDim>>());
}

template <class CDim>
DDC_INLINE_FUNCTION Coordinate<CDim> rmin(DiscreteDomain<FourierSampling<CDim>> const& d)
{
    return coordinate(d.front());
}

template <class CDim>
DDC_INLINE_FUNCTION Coordinate<CDim> rmax(DiscreteDomain<FourierSampling<CDim>> const& d)
{
    return coordinate(d.back());
}

template <class CDim>
DDC_INLINE_FUNCTION Coordinate<CDim> rlength(DiscreteDomain<FourierSampling<CDim>> const& d)
{
    return rmax(d) - rmin(d);
}

template <class T>
struct is_fourier_domain : std::false_type
{
};

template <class... DDims>
struct is_fourier_domain<DiscreteDomain<DDims...>>
    : std::conditional_t<(is_fourier_sampling_v<DDims> && ...), std::true_type, std::false_type>
{
};

template <class T>
constexpr bool is_fourier_domain_v = is_fourier_domain<T>::value;
