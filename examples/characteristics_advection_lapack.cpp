...
int main(int argc, char** argv)
{
    ...
    for (auto const iter :
		...
		ddc::Chunk coef_y(spline_builder.spline_domain(), ddc::HostAllocator<double>());
		ddc::Chunk last_density_y(x_domain, ddc::HostAllocator<double>());
		for (auto y : y_domain) {
		  for (auto x : x_domain) {
			last_density_y(x) = last_density(x,y);
		  }
		  spline_builder(coef_y.span_view(), last_density_y.span_view());
		  for (auto x : spline_builder.spline_domain()) {
			coef(x,y) = coef_y(x);
		  }
		}
        spline_evaluator(
