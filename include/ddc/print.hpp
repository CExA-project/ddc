...
inline void print_demangled_type_name(std::ostream& os, std::type_info const& type)
{
#if defined(KOKKOS_COMPILER_GNU) || defined(KOKKOS_COMPILER_CLANG)
    int status;

    std::unique_ptr<char, decltype(std::free)*> const
            demangled_name(abi::__cxa_demangle(type.name(), nullptr, nullptr, &status), std::free);
    if (status != 0) {
        ...
        ChunkSpan<ElementType, SupportType, LayoutStridedPolicy, MemorySpace> const& chunk_span)
{
    ...
    os << '\n';
    ddc::detail::print_demangled_type_name(os, typeid(chunk_span));
    os << '\n';
