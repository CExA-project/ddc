...
{
private:
    static constexpr std::array<double, 10> tihomirov_error_bound_array {
            1.0 / 2.0,
            1.0 / 8.0,
            1.0 / 24.0,
            5.0 / 384.0,
...
private:
     ...
     *******************************************************************************/
    static double tihomirov_error_bound(double const cell_width, int degree, double const max_norm)
    {
...
public:
    ...
    double error_bound(double const cell_width, int const degree)
    {
        return tihomirov_error_bound(cell_width, degree, m_evaluator.max_norm(degree + 1));
    }

    double error_bound(
            double const cell_width1,
            double const cell_width2,
            int const degree1,
            int const degree2)
    {
    ...
    double error_bound_on_deriv(double const cell_width, int const degree)
    {
    ...
    double error_bound_on_int(double const cell_width, int const degree)
    {
        return tihomirov_error_bound(cell_width, degree + 1, m_evaluator.max_norm(degree + 1));
    }
};
