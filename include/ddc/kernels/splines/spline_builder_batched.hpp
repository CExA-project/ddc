...
public:
    ...
    derivs_domain_type const derivs_domain() const noexcept
    {
        return ddc::replace_dim_of<interpolation_mesh_type, deriv_type>(
                ...
                ddc::DiscreteDomain<deriv_type>(
                        ddc::DiscreteElement<deriv_type>(0),
                        ddc::DiscreteVector<deriv_type>(typename bsplines_type::degree() / 2)));
    }
    ...
    void compute_interpolant_degree1(
            ...
            ddc::ChunkSpan<double const, vals_domain_type, Layout, memory_space> vals) const;
...
template <class SplineBuilder, class... IDimX>
template <class Layout>
void SplineBuilderBatched<SplineBuilder, IDimX...>::compute_interpolant_degree1(
        ddc::ChunkSpan<double, spline_domain_type, Layout, memory_space> const spline,
        ddc::ChunkSpan<double const, vals_domain_type, Layout, memory_space> const vals) const
{
    ddc::for_each(
            ...
            DDC_LAMBDA(typename batch_domain_type::discrete_element_type j) {
                ...
                if constexpr (bsplines_type::is_periodic()) {
                    spline(ddc::DiscreteElement<bsplines_type>(
                           ...
                           j)
                            = spline(ddc::DiscreteElement<bsplines_type>(0), j);
                }
            });
}
