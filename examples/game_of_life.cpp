...
void blinker_init(DiscreteDomain<DDimX, DDimY> const& domain, Chunk<cell, DiscreteDomain<DDimX, DDimY>>& cells) {
    for_each(domain, [&](DiscreteCoordinate<DDimX, DDimY> const ixy) {
        ...
        if (iy == DiscreteCoordinate<DDimY>(2) && (ix >= DiscreteCoordinate<DDimX>(1) && ix <= DiscreteCoordinate<DDimX>(3))) 
            cells(ixy) = true;
...
std::ostream& print_2DChunk(std::ostream& os, ChunkSpan<ElementType, DiscreteDomain<DDimX, DDimY>> chunk) {
    for_each(select<DDimY>(chunk.domain()), [&](DiscreteCoordinate<DDimY> const iy) {
        for_each(select<DDimX>(chunk.domain()), [&](DiscreteCoordinate<DDimX> const ix) {
            os << (chunk(ix, iy) ? "*" : ".");
        });
        os << "\n";
    });
...
int main()
{
    ...
    for (; iter < nt; ++iter) {
        ...
        for_each(cells_in.domain(), [&](DiscreteCoordinate<DDimX, DDimY> const ixy) {
            DiscreteCoordinate<DDimX> const ix = select<DDimX>(ixy);
            DiscreteCoordinate<DDimY> const iy = select<DDimY>(ixy);
            ...
            if (cells_in(ixy)) {
                alive_neighbors--;
                if (alive_neighbors < 2 || alive_neighbors > 3) cells_out(ixy) = false;
            }
            else {
                if (alive_neighbors == 3) cells_out(ixy) = true;
            }
        });
