 ...
 */
template <class DDim>
DiscreteDomain<DDim> create_periodic_sampling(
        ...
        DiscreteVector<DDim> n_period)
{
    ...
    assert(n > 1);
    assert(n_period > 1);
    init_discrete_space_from_impl<DDim>(typename DDim::template Impl<DDim, Kokkos::HostSpace>(
            a,
            Coordinate<typename DDim::continuous_dimension_type>((b - a) / (n - 1)),
            n_period));
