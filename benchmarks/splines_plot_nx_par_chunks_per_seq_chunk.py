...
data_groups = {nx: {"par_chunks_per_seq_chunk": [], "bytes_per_second": [], "gpu_mem_occupancy": []} for nx in nx_values}
...
for nx, group_data in data_groups.items():
    par_chunks_per_seq_chunk = group_data["par_chunks_per_seq_chunk"]
    gpu_mem_overhead = [group_data["gpu_mem_occupancy"][i]-nx*100000*8 for i in range(len(par_chunks_per_seq_chunk))]
    plt.plot(par_chunks_per_seq_chunk, gpu_mem_overhead, marker='o', markersize=5, label=f'nx={nx}')
