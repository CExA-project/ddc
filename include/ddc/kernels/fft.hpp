...
public:
    explicit RescaleFn(T coef) noexcept : m_coef(std::move(coef)) {}

    template <class U>
    KOKKOS_FUNCTION [[nodiscard]] U operator()(U const& value) const noexcept
    {
        return m_coef * value;
    }
};
        ...
        KwArgsImpl const& kwargs)
{
    ...
    if (kwargs.normalization == ddc::FFT_Normalization::FULL) {
        ...
        ddc::parallel_transform(exec_space, out, RescaleFn<real_type_t<Tout>>(norm_coef));
    }
}
