...
struct RescaleBy2
{
    KOKKOS_FUNCTION [[nodiscard]] int operator()(int const value) const noexcept
    {
        return 2 * value;
    }
};
...
TEST(ParallelTransform, OneDimension)
{
    ...
    ddc::parallel_transform(view, RescaleBy2());
...
TEST(ParallelTransform, OneDimensionWithExecutionSpace)
{
    ...
    ddc::parallel_transform(exec_space, view, RescaleBy2());
    exec_space.fence();
    EXPECT_EQ(std::count(storage.begin(), storage.end(), 2), dom.size());
}
