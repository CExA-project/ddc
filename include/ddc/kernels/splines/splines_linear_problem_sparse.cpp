...
class SplinesLinearProblemSparse::Impl : public SplinesLinearProblem
{
private:
...
public:
    ...
    explicit Impl(
        ...
        , m_cols_per_chunk(cols_per_chunk.value_or(default_cols_per_chunk<Kokkos::Serial>()))
        , m_preconditioner_max_block_size(preconditioner_max_block_size.value_or(
                  default_preconditioner_max_block_size<Kokkos::Serial>()))
    {
        std::shared_ptr const gko_exec = gko::ext::kokkos::create_executor(Kokkos::Serial());
    ...
    void solve(MultiRHS b, bool transpose) const override
    {
        ...
        Kokkos::View<double**, Kokkos::LayoutRight, Kokkos::Serial> const
...
/**
     ...
     * used by the block-Jacobi preconditioner. see default_preconditioner_max_block_size.
     */
SplinesLinearProblemSparse::SplinesLinearProblemSparse(
    ...
    , m_impl(std::make_unique<
              SplinesLinearProblemSparse::
                      Impl>(mat_size, cols_per_chunk, preconditioner_max_block_size))
{
}

SplinesLinearProblemSparse::~SplinesLinearProblemSparse() = default;

double SplinesLinearProblemSparse::get_element(std::size_t i, std::size_t j) const
{
    return m_impl->get_element(i, j);
}

void SplinesLinearProblemSparse::set_element(std::size_t i, std::size_t j, double aij)
{
    m_impl->set_element(i, j, aij);
}

void SplinesLinearProblemSparse::setup_solver()
{
    m_impl->setup_solver();
}

void SplinesLinearProblemSparse::solve(MultiRHS const b, bool const transpose) const
{
