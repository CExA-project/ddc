...
public:
    ...
    void operator()([[maybe_unused]] index_type<0> unused_id) const
    {
        m_f(DiscreteElement<>());
    }
    ...
    KOKKOS_FUNCTION void operator()(
            ...
            [[maybe_unused]] index_type<0> unused_id) const
    {
        m_f(DiscreteElement<>());
    }

    template <std::size_t N = sizeof...(Idx), std::enable_if_t<(N > 0), bool> = true>
    void operator()(index_type<Idx>... ids) const
    {
        m_f(m_support(typename Support::discrete_vector_type(ids...)));
    }

    template <std::size_t N = sizeof...(Idx), std::enable_if_t<(N > 0), bool> = true>
    KOKKOS_FUNCTION void operator()(use_annotated_operator, index_type<Idx>... ids) const
    {
        m_f(m_support(typename Support::discrete_vector_type(ids...)));
    }
};
