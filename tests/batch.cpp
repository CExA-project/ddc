        ...
        ddc::ChunkSpan<Tin const, DDomIn, LayoutIn, MemorySpace> const& in)
{
    ...
    ddc::for_each(out.domain(), [&](typename DDomOut::discrete_element_type iout) {
        ...
        ddc::ChunkSpan<Tin const, DDomIn, LayoutIn, MemorySpace> const& in)
{
    ...
    ddc::parallel_for_each(
            ...
            KOKKOS_LAMBDA(typename DDomIn::discrete_element_type iin) {
                ...
                ddc::for_each(ddom_batch, [&](typename DDomBatch::discrete_element_type ib) {
                    slice_out(ib) = val;
                });
            });
    // ddc::parallel_for_each(
    ...
    ddc::for_each(
            ddom_batch,
            KOKKOS_LAMBDA(typename DDomBatch::discrete_element_type ib) {
